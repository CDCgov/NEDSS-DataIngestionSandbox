...
def generate_reason_for_visit_data(num_rows):
    reasons = ["Routine Checkup", "Flu Symptoms", "Back Pain", "Headache", "Allergy Testing", "Diabetes Management",
               "Cardiac Screening", "Skin Rash", "Fever", "Post-Surgery Follow-up", "Vaccination",
...
def generate_history_of_present_illness_data(num_rows):
    reasons = ["Chest Pain", "Shortness of Breath", "Cough (Productive/Non-productive)", "Fever and Chills",
               "Abdominal Pain", "Headache", "Dizziness or Lightheadedness", "Fatigue or Weakness",
               "Nausea and Vomiting", "Diarrhea or Constipation", "Muscle or Joint Pain", "Skin Rash or Lesion",
               "Unexplained Weight Loss", "Palpitations", "Sore Throat", "Urinary Frequency or Burning",
               "Vision Changes or Eye Pain", "Hearing Loss or Ear Pain", "Anxiety or Depression",
               "Memory Loss or Cognitive Decline", "Insomnia or Sleep Disturbances", "Back Pain", "Leg Swelling",
               "Numbness or Tingling", "Bleeding or Bruising", "Allergic Reaction", "Post-surgical Pain",
               "Follow-up for Chronic Illness", "Loss of Appetite", "Difficulty Swallowing (Dysphagia)"]
    data = []
    if num_rows == 0:
...
def generate_plan_of_treatment_data(num_rows):
    ...
    if num_rows == 0:
        return f"<text>No Plan of Treatment information available</text>"
...
def generate_immunizations_data(num_rows):
    vaccines = ["DTaP, 5 pertussis antigens vaccine (DAPTACEL)", "Hep B, adult vaccine (Engerix/Recombivax)",
                ...
                "MMR (Measles, Mumps, Rubella) Vaccine", "Varicella (Chickenpox) Vaccine",
                "Pneumococcal Vaccine (PCV13)", "Flu Vaccine", "HPV Vaccine (Gardasil)",
                "Tetanus, Diphtheria, Pertussis (Tdap) Vaccine", "Hepatitis A Vaccine", "Meningococcal Vaccine"]
    if num_rows == 0:
...
def generate_results_data(num_rows):
    lonic_code_data = [{"code": "102048-6", "test_name": "SARS coronavirus 2 and Respiratory syncytial virus Ag"},
                       ...
                       {"code": "41459-9", "test_name": "SARS coronavirus"},
                       {"code": "94763-0", "test_name": "SARS coronavirus 2"},
                       {"code": "95209-3", "test_name": "SARS coronavirus+SARS coronavirus 2 Ag"},
                       {"code": "15868-3", "test_name": "Mussel Ab.IgE.RAST class"},
                       {"code": "6184-6", "test_name": "Mussel Ab.IgE"}, {"code": "104651-5",
    ...
    if num_rows == 0:
        return f"<text>No Lab Test Results information available</text>"
...
def generate_medications_administered_data(num_rows):
    ...
    if num_rows == 0:
        return f"<text>No Medications Administered or information not available</text>"
...
def generate_eicr(eicr_template, num_conditions=0):
    ...
    return patient_first_name, patient_last_name, eicr_template.format(id_extension=generate_extension_id(),
                                                                       uuid=generate_uuid(),
                                                                       setid_extension_uuid=generate_uuid(),
                                                                       setid_root=generate_unique_root(),
                                                                       patient_id=generate_patient_id(),
                                                                       patient_root=generate_unique_root(),
                                                                       ssn=fake.ssn(), title=generate_title(),
                                                                       first_name=patient_first_name,
                                                                       last_name=patient_last_name,
                                                                       birth_time=generate_birthtime(),
                                                                       gender_code=gender_code, gender=gender,
                                                                       street_address=fake.street_address(),
                                                                       city=fake.city(), state=fake.state_abbr(),
                                                                       zip=fake.zipcode(), country="USA",
                                                                       phone_number=generate_phone_number(),
                                                                       email=patient_first_name + "." + patient_last_name + "@example.com",
                                                                       race_code=race['code'],
                                                                       race_display_name=race['displayName'],
                                                                       ethnicity_code=ethnicity['code'],
                                                                       ethnicity_display_name=ethnicity['displayName'],
                                                                       visit_date=generate_effective_time(),
                                                                       assigned_author_id=generate_unique_root(),
                                                                       author_name=author_name,
                                                                       author_id=generate_uuid(),
                                                                       author_first_name=author_first_name,
                                                                       author_last_name=author_last_name,
                                                                       author_street_address=fake.street_address(),
                                                                       author_city=fake.city(),
                                                                       author_state=fake.state_abbr(),
                                                                       author_zip=fake.zipcode(), author_country="USA",
                                                                       author_phone_number=generate_phone_number(),
                                                                       ...
                                                                       location_code=generate_extension_id(),
                                                                       ...
                                                                       history_of_present_illness_data=generate_history_of_present_illness_data(
                                                                           random.randint(0, 10)),
                                                                       plan_of_treatment_data=generate_plan_of_treatment_data(
                                                                           random.randint(0, 10)),
                                                                       immunizations_data=generate_immunizations_data(
                                                                           random.randint(0, 10)),
                                                                       results_data=generate_results_data(
                                                                           random.randint(0, 10)),
                                                                       encounters_data=generate_encounters_data(1),
                                                                       problems_data=generate_problems_data(),
                                                                       medications_administered_data=generate_medications_administered_data(
                                                                           random.randint(0, 5)))



def validate_num_eicrs(value):
    try:
        int_value = int(value)
        if int_value < 1 or int_value > 10000:
            raise argparse.ArgumentTypeError("Number of eICRs must be between 1 and 10,000.")
        return int_value
    except ValueError:
        raise argparse.ArgumentTypeError("Number of eICRs must be an integer.")


if __name__ == "__main__":
    current_dir = os.path.dirname(os.path.abspath(__file__))

    template_file = os.path.join(current_dir, "template", "eICR-CCDA-template.xml")

    parser = argparse.ArgumentParser(description="Generate EICRs with optional parallel processing.")
    parser.add_argument(
        "--num_eicrs",
    ...
    print("Sample eICRs generated and saved to: " + output_dir)
