...
#################################################################
##### Arguments if we are running this program independetly #####
#################################################################
# numELRs = int(sys.argv[1])
...
class HL7Generator:
    def __init__(self):
        ...
        self.race_df = None
        self.assign_df = None
        self.sending_df = None
        self.disease_df = None
        self.reason_df = None
        self.specimen_df = None
        self.alt_spm_df = None

    def set_condition_code(self, conditionCode):
        self.conditionCode = str(conditionCode)
    ...
    ####################################################################
    # Caching all the SQL queries from the new HL7 Generator Database: #
    ####################################################################
    ...
    def generateELR(self, numELRs, conditionCode):
        ###############################################################
        # Replacing this loop by adding the loop in the service query #
        #
        #for i in range(int(numELRs)):        
        #
        ###############################################################
            
        self.numELRs = numELRs
        ...
        ####################################################################
        ## Currently DTS1 database is only accepting the bewlow 3 values ##
        ####################################################################
        ...
        race_random_row = self.race_df.sample(n=1)
        ...
        auth_random_row = self.assign_df.sample(n=1)
        ...
        send_auth_random_row = self.sending_df.sample(n=1)
        ...
        disease_random_row = self.disease_df.sample(n=1)
        ...
        reason_random_row = self.reason_df.sample(n=1)
        ...
        specimen_random_row = self.specimen_df.sample(n=1)
    ...
    #       ##############################################################################################################
    #       #### This code is to Create a separate text file for each message, when running this script independently ####
    #       ##############################################################################################################
    #
